<?php
    /*  Cela assure que la session est démarrée */
    session_start();

    include './pokemon_api.php';

     /* Permet de récupérer l'ID du dresseur connecté */
    $collectorId = isset($_SESSION['collector_id']) ? $_SESSION['collector_id'] : null;

    /* // Cela vérifie si le dresseur est connecté */
    if ($collectorId) {
        /* cela permet de récupérer la liste des Pokémon du dresseur */
        $pokemonList = list_all_pokemon_from_collector($collectorId);
...
<a href="./collection.php" class="collection active">Collection</a>
<a href="./detail.php" class="detail">Détail</a>
<a href="./index.php?logout" class="se_connecter">Se déconnecter</a>
      </nav>
      <section>
        <p>Voici votre collection avec PokeCollector</p>
      </section>
      </header>
      <main>
      ...
      <div class="barre_recherche">
        <div class="inputBox_container">
          ...
          <input class="inputBox" id="inputBox" type="text" placeholder="Nom du pokemon....." oninput="searchCollection()">
        </div>
      </div>

      <a href="./catalogue.php"><button>Accèder au catalogue</button></a>
...
<?  /* Affichage de la collection */
        foreach ($pokemonList as $association) {
            $pokemonId = $association['pokemon_id'];
            $nickname = isset($association['pokemon_nickname']) ? $association['pokemon_nickname'] : '';
            $associationId = $association['association_id'];

            /*  Cela récupére les détails du Pokémon */
            $pokemonData = get_pokemon_by_id($pokemonId);
        ?>
        ...
        <div class="catalogue_card">
            <div class="catalogue_card-info">
                <a href="./detail.php?id=<?php echo $pokemonId; ?>">
                    <div class="catalogue_card-avatar">
                        <img src="./Assets/Image/img/full/<?php echo $pokemonId; ?>.png" alt="<?php echo $nickname; ?>">
                    </div>
                </a>
                <div class="catalogue_card-title"><?php echo $pokemonData['identifier']; ?></div>
                <div class="catalogue_card-subtitle"><?php echo $nickname; ?></div>
        ...
        <li class="catalogue_card-social__item">         
          ...
          <a href="./remove_from_collection.php?id=<?=$associationId;?>">
          <li class="catalogue_card-social__item">
            <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
        ...
        <?php
         ...
         /* Cela gère le cas où le dresseur n'est pas connecté */
        echo "Vous n'êtes pas connecté. Veuillez vous connecter pour voir votre collection.";
    }
    ?>
...
function searchCollection() {
   /* valeur de la barre de recherche */
    var searchTerm = document.getElementById('inputBox').value.toLowerCase();
    
    /*  éléments de carte de la collection */
    var collectionCards = document.querySelectorAll('.catalogue_card');
    
    /*  Parcoure chaque carte de la collection */
    collectionCards.forEach(function(card) {
       /*   Récupére le nom du Pokémon dans la carte */
        var pokemonName = card.querySelector('.catalogue_card-title').innerText.toLowerCase();
        
/*          Affiche ou masque la carte en fonction de la correspondance avec le terme de recherche */ 
